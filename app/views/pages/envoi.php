...
?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Check if a session alert exists
            const sessionAlert = `<?= isset($_SESSION['alert']) ? $_SESSION['alert'] : '' ?>`;
            if (sessionAlert) {
                // Display SweetAlert based on the session alert
                Swal.fire({
                    title: sessionAlert.includes('success') ? 'Success!' : 'Error!',
                    text: sessionAlert,
                    icon: sessionAlert.includes('success') ? 'success' : 'error',
