...
-- Création de la table wallets en tant que tableau associatif entre users et cryptos
CREATE TABLE wallets
(
    user_id   INT REFERENCES users (id) ON DELETE CASCADE,
    crypto_id INT REFERENCES cryptos (id) ON DELETE CASCADE,
    balance   DECIMAL(18, 8) DEFAULT 0, -- Quantité détenue
    PRIMARY KEY (user_id, crypto_id)    -- Clé primaire composite pour garantir unicité
);
